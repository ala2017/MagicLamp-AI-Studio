...
description: "AI 编程过程中的文件安全中间件——每次编辑文件前自动执行安全编辑（备份→写入→验证→失败回滚），多文件修改前自动打 git checkpoint，会话启动时自动运行完整性扫描。让 AI 永远不会因为会话中断、网络断连、token 耗尽而丢失你的代码。当你让 AI 编写、修改、编辑、修复、重构任何文件时，当你开始一个涉及多文件的复杂任务时，当你恢复一个中断的会话时，都要使用这个 skill。IronFile 是 AI 文件操作的安全阀——不要跳过它。"
